...
#Please use crontab scheduling to make this script run weekly
...
function darbackup(){
    ...
    archive="$2.personal "
    printf "$(date -u +%Y-%m-%d-%H%M):\n" >> ${LOG_DIR}
    if ! stderr="$(dar $args $archive -R $SOURCE_USR >>${LOG_DIR})"
    then
        echo "dar shared data failed"
        tail -1 "$BACKDIR/${LOG_DIR}"
        mail -s "$(date +%Y-%m-%d): Backup error occurred" $EMAIL < $SCRIPT_DIR/${LOG_DIR}
    else
        echo "dar personal data success " >> ${LOG_DIR}
    fi
    cat $BACKDIR/${LOG_DIR}
}
